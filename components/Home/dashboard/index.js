...
export default function Dashboard() {
    ...
    const withEmailCount = data.filter(row => row.email).length;
    const withoutEmailCount = data.filter(row => !row.email).length;
                ...
                <Grid container spacing={5} mt={1}>
                        ...
                        <Grid container spacing={3}>
                            <Grid item xs={12} sm={12} md={6}>
                                <Paper sx={{ padding: '1.5rem', textAlign: 'start', backgroundColor: '#00AC69', color: '#fff', position: 'relative' }}>
                                    <Typography variant="h4" sx={{ fontSize: { xs: '1.5rem', md: '2rem' } }}>{activeCount}</Typography>
                            ...
                            <Grid item xs={12} sm={12} md={6}>
                                <Paper sx={{ padding: '1.5rem', textAlign: 'start', backgroundColor: '#FC766A', color: '#fff', position: 'relative' }}>
                                    <Typography variant="h4" sx={{ fontSize: { xs: '1.5rem', md: '2rem' } }}>{inactiveCount}</Typography>
                            ...
                            <Grid item xs={12} sm={12} md={6}>
                                <Paper sx={{ padding: '1.5rem', textAlign: 'start', backgroundColor: '#2C3E50', color: '#fff', position: 'relative' }}>
                                    <Typography variant="h4" sx={{ fontSize: { xs: '1.5rem', md: '2rem' } }}>{withEmailCount}</Typography>
                            ...
                            <Grid item xs={12} sm={12} md={6}>
                                <Paper sx={{ padding: '1.5rem', textAlign: 'start', backgroundColor: '#5B84B1', color: '#fff', position: 'relative' }}>
                                    <Typography variant="h4" sx={{ fontSize: { xs: '1.5rem', md: '2rem' } }}>{withoutEmailCount}</Typography>
                    ...
                    <Grid item xs={12} md={6}>
                        <Box sx={{ backgroundColor: '#fff', borderRadius: '5px', boxShadow: '0px 3px 15px #00000012' }}>
                            ...
                            <TableContainer component={Paper} sx={{ boxShadow: 'none', padding: '0 2rem 1rem', maxHeight: '220px', overflowY: 'auto' }}>
                                    ...
                                    <TableBody>
                                        {data.map((row) => (
                                            <TableRow key={row.id}>
                                                ...
                                                <TableCell align="left" sx={{ borderBottom: 'none', opacity: 0.4, fontSize: '10px', padding: '5px' }}>
